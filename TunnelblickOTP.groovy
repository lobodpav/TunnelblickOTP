...
String executeCommand(String[] command, String errorMessage = "", boolean failOnError = true) {
    ...
    // ensures the process will never block because its output is bigger than the limited internal buffer size
    // a blocking example would be this command: ["seq", "-f", "#", "-s", "", "100000"]
    exec.waitForProcessOutput(stdout, stderr)
    def code = exec.exitValue()

    if (code != 0) {
        System.err.println((errorMessage.empty ? "Error: " : "${errorMessage}: ") + stderr)
